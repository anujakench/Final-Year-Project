...
struct timespec t1,t2,t4; //structures for storing nanosecond time
...
void set_time(struct timespec set)
{
	...
	if((i=ioctl(rawsock,SIOCDEVPRIVATE+1,&set))<0)
	{
		printf("\n\tError in ioctl");
		exit(0);
	}
...
{ 
	...
	struct itimerval timer;
    	...
    	FILE *fp;
	pcap_t *descr1;//packet descriptor
   ...
   //	sleep(20); //slave2 in sleep for 30 seconds
	...
	alarm(TIMEOUT_SECS);
		pcap_setdirection(descr,PCAP_D_IN);  //capture only incoming packets	
	...
	pcap_close(descr); //close the packet descriptor
	...
	pcap_close(descr); //close packet descriptor	
